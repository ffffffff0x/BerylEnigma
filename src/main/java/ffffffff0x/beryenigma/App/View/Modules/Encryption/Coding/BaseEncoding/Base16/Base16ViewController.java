...
public class Base16ViewController extends BaseEncodingViewController {
    ...
    protected String encodeSplitToString() {
        try {
            return Coding_Base16.encodeSplitToString(JTA_src.getText(),JCB_charset.getValue().toString(),JTF_split.getText());
    ...
    protected String decodeSplitToString() {
        try {
            return Coding_Base16.decodeSplitToString(JTA_src.getText(),JCB_charset.getValue().toString(),JTF_split.getText());
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            JTA_dst.validate();
        }
